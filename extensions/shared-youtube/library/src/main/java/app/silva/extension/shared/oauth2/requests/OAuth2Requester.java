...
package app.silva.extension.shared.oauth2.requests;

import static app.silva.extension.shared.StringRef.str;
import static app.silva.extension.shared.oauth2.requests.OAuth2Routes.getJsonConnectionFromRoute;
import static app.silva.extension.shared.oauth2.requests.OAuth2Routes.getUrlConnectionFromRoute;
...
public class OAuth2Requester {
    ...
    private static void clearAll(boolean clearedByUser) {
        if (!clearedByUser) {
            Utils.showToastShort(str("silva_oauth2_toast_invalid"));
        }
        ...
        Utils.showToastShort(str("silva_oauth2_toast_reset"));
    }
    ...
    public static ActivationCodeData getActivationCodeData() {
        ...
        synchronized (OAuth2Requester.class) {
            try {
                ...
                handleConnectionError(str("silva_oauth2_connection_failure_status", responseCode), null);
            } catch (SocketTimeoutException ex) {
                handleConnectionError(str("silva_oauth2_connection_failure_timeout"), ex);
            } catch (IOException ex) {
                handleConnectionError(str("silva_oauth2_connection_failure_generic"), ex);
            } catch (Exception ex) {
                    ...
                    if (json.has(errorKey)) {
                        ...
                        if (error.equalsIgnoreCase("authorization_pending")) {
                            if (showErrorToastIfAccessNotApproved) {
                                Utils.showToastLong(str("silva_oauth2_connection_failure_auth_not_approved"));
                            }
                            return null;
                        }

                        Utils.showToastLong(str("silva_oauth2_connection_failure_auth_error", error));
                        return null;
                    }
    ...
    private static void updateAccessTokenData(String refreshToken) {
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                ...
                handleConnectionError(str("silva_oauth2_connection_failure_status_400", responseCode), null);
                clearAll(false);
            } else {
                handleConnectionError(str("silva_oauth2_connection_failure_status", responseCode), null);
            }
