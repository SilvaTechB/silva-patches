package app.silva.extension.shared;
...
public class Utils {
     ...
     *
     * @param packageName The application package name to check (e.g., "app.silva.android.apps.youtube.music").
     ...
     *         {@link #isRightToLeftLocale(Locale)} with {@link BaseSettings#SILVA_LANGUAGE}.
     *         This is the default locale of the device, which may differ if
     *         {@link BaseSettings#SILVA_LANGUAGE} is set to a different language.
     */
     ...
     *         {@link BaseSettings#SILVA_LANGUAGE}.
     */
    ...
    public static String removePunctuationToLowercase(@Nullable CharSequence original) {
        ...
        return PUNCTUATION_PATTERN.matcher(original).replaceAll("")
                .toLowerCase(BaseSettings.SILVA_LANGUAGE.get().getLocale());
    }
    ...
    private static Collator getCollator() {
        Locale currentLocale = BaseSettings.SILVA_LANGUAGE.get().getLocale();
    ...
    public static void sortPreferenceGroups(PreferenceGroup group) {
        ...
        for (Pair<String, Preference> pair : preferences) {
            ...
            if (pref instanceof PreferenceScreen || pref instanceof SilvaAboutPreference
                    || pref.getIntent() != null) {
