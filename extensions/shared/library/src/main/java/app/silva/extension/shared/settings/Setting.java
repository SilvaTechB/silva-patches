...
public abstract class Setting<T> {
     ...
     */
    public static final SharedPrefCategory preferences = new SharedPrefCategory("silva_prefs");
     ...
     */
    private static final String OPTIONAL_SILVA_SETTINGS_PREFIX = "silva_";

    /**
     * The path, minus any 'morphe' prefix to keep JSON concise.
     */
    private String getImportExportKey() {
        if (key.startsWith(OPTIONAL_SILVA_SETTINGS_PREFIX)) {
            return key.substring(OPTIONAL_SILVA_SETTINGS_PREFIX.length());
        }
        return key;
    }
    ...
    public static boolean importFromJSON(Activity alertDialogContext, String settingsJsonString) {
        try {
            ...
            String resetKey = "silva_settings_import_reset";
            ...
            if (ResourceUtils.getIdentifier(ResourceType.STRING, resetKey) != 0) {
                ...
                Utils.runOnMainThreadDelayed(() -> Utils.showToastLong(numberOfSettingsImportedFinal == 0
                                ? str(resetKey)
                                : str("silva_settings_import_success", numberOfSettingsImportedFinal)),
                        150);
            }

            return rebootSettingChanged;
        } catch (JSONException | IllegalArgumentException ex) {
            String failureKey = "silva_settings_import_failure_parse";
