...
import static app.silva.extension.shared.settings.preference.about.SilvaAboutPreference.CREDITS_LINK;
...
class AboutRoutes {
     ...
     */
    private static final List<SilvaAboutPreference.WebLink> NO_CONNECTION_STATIC_LINKS = List.of(
            new SilvaAboutPreference.WebLink(true, "Website", null, "https://morphe.software"),
            CREDITS_LINK
    );
    ...
    private static final Route.CompiledRoute GITHUB_ROUTE_PATCHES = new Route(GET,
            (Utils.isPreReleasePatches()
                    ? "/SilvaTechB/silva-patches/refs/heads/dev/patches-bundle.json"
                    : "/SilvaTechB/silva-patches/refs/heads/main/patches-bundle.json")
    ).compile();
    ...
    @Nullable
    private static volatile List<SilvaAboutPreference.WebLink> fetchedLinks;

    static boolean hasFetchedLinks() {
        return fetchedLinks != null;
    }

    static List<SilvaAboutPreference.WebLink> fetchAboutLinks() {
        try {
            ...
            List<SilvaAboutPreference.WebLink> links = new ArrayList<>();

            JSONArray donations = json.getJSONObject("donations").getJSONArray("links");
            for (int i = 0, length = donations.length(); i < length; i++) {
                SilvaAboutPreference.WebLink link = new SilvaAboutPreference.WebLink(donations.getJSONObject(i));
                if (link.preferred) {
                    links.add(link);
                }
            }
            ...
            for (int i = 0, length = socials.length(); i < length; i++) {
                SilvaAboutPreference.WebLink link = new SilvaAboutPreference.WebLink(socials.getJSONObject(i));
                links.add(link);
            }
