...
public abstract class AbstractPreferenceFragment extends PreferenceFragment {
    ...
    protected void initialize() {
        ...
        if (BaseSettings.SHOW_MENU_ICONS.get()) {
            preferenceResourceName = Utils.appIsUsingBoldIcons()
                    ? "silva_prefs_icons_bold"
                    : "silva_prefs_icons";
        } else {
            preferenceResourceName = "silva_prefs";
        }
    ...
    private void showSettingUserDialogConfirmation(Preference pref, Setting<?> setting) {
        ...
        if (confirmDialogTitle == null) {
            confirmDialogTitle = str("silva_settings_confirm_user_dialog_title");
        }
    ...
    public static void showRestartDialog(Context context) {
        ...
        if (restartDialogTitle == null) {
            restartDialogTitle = str("silva_settings_restart_title");
        }
        if (restartDialogMessage == null) {
            restartDialogMessage = str("silva_settings_restart_dialog_message");
        }
        if (restartDialogButtonText == null) {
            restartDialogButtonText = str("silva_settings_restart");
        }
    ...
    public void showImportExportTextDialog() {
        try {
            ...
            Button btnExport = createDialogButton(context, str("silva_settings_export_file"), 0, margin, v -> exportActivity());
            Button btnImport = createDialogButton(context, str("silva_settings_import_file"), margin, 0, v -> importActivity());
    ...
    private void exportTextToFile(android.net.Uri uri) {
        try {
            ...
            if (out != null) {
                ...
                showLocalizedToast("silva_settings_export_file_success", "Settings exported successfully");
            }
        } catch (Exception e) {
            showLocalizedToast("silva_settings_export_file_failed", "Failed to export settings");
        ...
        try {
            ...
            if (in != null) {
                ...
                if (currentImportExportEditText != null) {
                    ...
                    showLocalizedToast("silva_settings_import_file_success", "Settings imported successfully, tap Save to apply");
                } else {
                    importSettingsText(getContext(), result);
                }
            }
        } catch (Exception e) {
            showLocalizedToast("silva_settings_import_file_failed", "Failed to import settings");
