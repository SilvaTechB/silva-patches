...
import app.silva.extension.youtube.sponsorblock.ui.SponsorBlockPreferenceGroup;
...
public class SponsorBlockSettings {
    ...
    public static void importDesktopSettings(@NonNull String json) {
        ...
        try {
            ...
            Utils.showToastLong(str("silva_sb_settings_import_successful"));
        } catch (Exception ex) {
            Logger.printInfo(() -> "failed to import settings", ex); // Use info level, as we are showing our own toast.
            Utils.showToastLong(str("silva_sb_settings_import_failed", ex.getMessage()));
        }
    }
    ...
    public static String exportDesktopSettings() {
        ...
        try {
            ...
            Utils.showToastLong(str("silva_sb_settings_export_failed", ex));
            return "";
        }
    }
    ...
    private static void showExportWarningIfNeeded(@Nullable Activity activity) {
        ...
        if (activity != null && SponsorBlockSettings.userHasSBPrivateID()
            ...
            Pair<Dialog, LinearLayout> dialogPair = CustomDialog.create(
                    activity,
                    null, // No title.
                    str("silva_sb_settings_silva_export_user_id_warning"), // Message.
                    null, // No EditText.
                    null, // OK button text.
                    () -> {}, // OK button action (dismiss only).
                    null, // No cancel button action.
                    str("silva_sb_settings_silva_export_user_id_warning_dismiss"), // Neutral button text.
