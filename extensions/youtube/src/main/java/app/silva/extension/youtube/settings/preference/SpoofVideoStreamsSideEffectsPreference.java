...
package app.silva.extension.youtube.settings.preference;
    ...
    private void updateUI() {
        ...
        switch (clientType) {
            case ANDROID_CREATOR ->
                    summary = str("silva_spoof_video_streams_about_no_audio_tracks")
                            + '\n' + str("silva_spoof_video_streams_about_no_stable_volume")
                            + '\n' + str("silva_spoof_video_streams_about_no_av1")
                            + '\n' + str("silva_spoof_video_streams_about_no_force_original_audio");
            case ANDROID_REEL ->
                    summary = str("silva_spoof_video_streams_about_playback_failure");
            // VR 1.65 is not exposed in the UI and should never be reached here.
            case ANDROID_VR_1_64, ANDROID_VR_1_65 ->
                    summary = str("silva_spoof_video_streams_about_no_audio_tracks")
                            + '\n' + str("silva_spoof_video_streams_about_no_stable_volume");
            case TV ->
                    summary = str("silva_spoof_video_streams_about_js");
            case TV_SIMPLY ->
                    summary = str("silva_spoof_video_streams_about_js")
                            + '\n' + str("silva_spoof_video_streams_about_playback_failure");
            case VISIONOS ->
                    summary = str("silva_spoof_video_streams_about_experimental")
                            + '\n' + str("silva_spoof_video_streams_about_no_audio_tracks")
                            + '\n' + str("silva_spoof_video_streams_about_no_av1");
        ...
        if (!clientType.name().startsWith("ANDROID_VR") && clientType != ClientType.ANDROID_REEL) {
            summary += '\n' + str("silva_spoof_video_streams_about_no_immersive_mode");
        }
