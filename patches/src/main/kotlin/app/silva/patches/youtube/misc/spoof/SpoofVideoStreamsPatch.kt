package app.silva.patches.youtube.misc.spoof

import app.silva.patches.shared.misc.settings.preference.ListPreference
import app.silva.patches.shared.misc.settings.preference.NonInteractivePreference
import app.silva.patches.shared.misc.settings.preference.PreferenceScreenPreference
import app.silva.patches.shared.misc.settings.preference.SwitchPreference
import app.silva.patches.shared.misc.settings.preference.TextPreference
import app.silva.patches.shared.misc.spoof.spoofVideoStreamsPatch
import app.silva.patches.youtube.misc.playservice.is_19_34_or_greater
import app.silva.patches.youtube.misc.playservice.is_20_03_or_greater
import app.silva.patches.youtube.misc.playservice.is_20_10_or_greater
import app.silva.patches.youtube.misc.playservice.is_20_14_or_greater
import app.silva.patches.youtube.misc.playservice.is_20_39_or_greater
import app.silva.patches.youtube.misc.playservice.versionCheckPatch
import app.silva.patches.youtube.misc.settings.PreferenceScreen
import app.silva.patches.youtube.misc.settings.settingsPatch
import app.silva.patches.youtube.shared.Constants.COMPATIBILITY_YOUTUBE
import app.silva.patches.youtube.shared.YouTubeActivityOnCreateFingerprint

val spoofVideoStreamsPatch = spoofVideoStreamsPatch(
    extensionClassDescriptor = "Lapp/morphe/extension/youtube/patches/spoof/SpoofVideoStreamsPatch;",
    mainActivityOnCreateFingerprint = YouTubeActivityOnCreateFingerprint,
    fixMediaFetchHotConfig = {
        is_19_34_or_greater
    },
    fixMediaFetchHotConfigAlternative = {
        // In 20.14 the flag was merged with 20.03 start playback flag.
        is_20_10_or_greater && !is_20_14_or_greater
    },
    fixParsePlaybackResponseFeatureFlag = {
        is_20_03_or_greater
    },
    fixMediaSessionFeatureFlag = {
        is_20_39_or_greater
     },

    block = {
        compatibleWith(COMPATIBILITY_YOUTUBE)

        dependsOn(
            userAgentClientSpoofPatch,
            settingsPatch,
            versionCheckPatch
        )
    },

    executeBlock = {

        PreferenceScreen.MISC.addPreferences(
            PreferenceScreenPreference(
                key = "silva_spoof_video_streams_screen",
                sorting = PreferenceScreenPreference.Sorting.UNSORTED,
                preferences = setOf(
                    SwitchPreference("silva_spoof_video_streams"),
                    ListPreference("silva_spoof_video_streams_client_type"),
                    NonInteractivePreference(
                        // Requires a key and title but the actual text is chosen at runtime.
                        key = "silva_spoof_video_streams_about",
                        summaryKey = null,
                        tag = "app.silva.extension.youtube.settings.preference.SpoofVideoStreamsSideEffectsPreference"
                    ),
                    NonInteractivePreference(
                        key = "silva_spoof_video_streams_sign_in_android_vr_about",
                        tag = "app.silva.extension.youtube.settings.preference.SpoofVideoStreamsSignInPreference",
                        selectable = true,
                    ),
                    SwitchPreference("silva_spoof_video_streams_av1"),
                    ListPreference("silva_spoof_video_streams_player_js_variant"),
                    SwitchPreference("silva_spoof_video_streams_disable_player_js_update"),
                    TextPreference("silva_spoof_video_streams_player_js_hash_value"),
                    SwitchPreference("silva_spoof_video_streams_stats_for_nerds"),
                )
            )
        )
    }
)
